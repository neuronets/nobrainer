...
class _ConvVariational(tf.keras.layers.Layer):
    ...
    def __init__(
            ...
            kernel_divergence_fn=(lambda q, p,
                                  ignore: kl_lib.kl_divergence(q, p)),
            ...
            **kwargs):
        super(_ConvVariational, self).__init__(activity_regularizer=activity_regularizer, **kwargs)
        self.rank = rank
        self.is_mc = is_mc
        self.filters = filters
        self.kernel_size = tf_layers_util.normalize_tuple(kernel_size,
                                                          rank, 'kernel_size')
        ...
        self.dilation_rate = tf_layers_util.normalize_tuple(dilation_rate,
                                                            rank, 'dilation_rate')
    ...
    def build(self, input_shape):
        ...
        if self.kernel_prior_fn is None:
            ...
            self.kernel_prior = self.kernel_prior_fn(dtype, kernel_shape,
                                                     'kernel_prior',
                                                     self.trainable,
                                                     self.add_variable)
        if self.bias_posterior_fn is None:
            ...
            self.bias_posterior = self.bias_posterior_fn(dtype, (self.filters,),
                                                         'bias_posterior',
                                                         self.trainable,
                                                         self.add_variable)
        if self.bias_prior_fn is None:
            ...
            self.bias_prior = self.bias_prior_fn(dtype, (self.filters,),
                                                 'bias_prior',
                                                 self.trainable,
                                                 self.add_variable)
    ...
    def _apply_variational_bias(self, inputs):
        ...
        if self.data_format == 'channels_first':
            ...
            if self.rank == 3:
                ...
                outputs_4d = tf.reshape(outputs, [outputs_shape[0],
                                                  outputs_shape[1],
    ...
    def _apply_divergence(self, divergence_fn, posterior,
                          prior, posterior_tensor, name):
        if (divergence_fn is None or posterior is None or prior is None):
            divergence = None
            return
        divergence = tf.identity(divergence_fn(posterior,
                                               prior, posterior_tensor), name=name)
...
class _ConvReparameterization(_ConvVariational):
    ...
    def _apply_variational_kernel(self, inputs):
        if (not isinstance(self.kernel_posterior, independent_lib.Independent) or not isinstance(self.kernel_posterior.distribution, normal_lib.Normal)):
            ...
            self.kernel_posterior_affine = normal_lib.Normal(loc=tf.zeros_like(self.kernel_posterior.distribution.loc), scale=self.kernel_posterior.distribution.scale)
            self.kernel_posterior_affine_tensor = (self.kernel_posterior_tensor_fn(self.kernel_posterior_affine))
            self.kernel_posterior_tensor = None
            outputs_m = self._convolution_op(inputs,
                                             self.kernel_posterior.distribution.loc)
            outputs_v = self._convolution_op(tf.square(inputs), tf.square(self.kernel_posterior.distribution.stddev()))
            ...
            if self.is_mc:
                err = (tf.sqrt(tf.add(outputs_v,
                                      tf.keras.backend.epsilon())) * outputs_e)
