...
def customIntensityScaling(x, y = None, trans_xy = False, scale_x=[0.0,1.0],  scale_y = None):
    ...
    minx = tf.cast(tf.convert_to_tensor(scale_x[0]*np.ones(x_norm.shape).astype(np.float32)), tf.float32)
    maxx = tf.cast(tf.convert_to_tensor(scale_x[1]*np.ones(x_norm.shape).astype(np.float32)), tf.float32)
    ...
    if trans_xy:
        ...
        y = tf.cast(y, tf.float32)
        miny = tf.cast(tf.convert_to_tensor(scale_y[0]*np.ones(y_norm.shape).astype(np.float32)), tf.float32)
        maxy = tf.cast(tf.convert_to_tensor(scale_y[1]*np.ones(y_norm.shape).astype(np.float32)), tf.float32)
...
def intensityMasking(x, mask_x, y = None,   trans_xy = False, mask_y = None):
    ...
    if trans_xy: 
        ...
        if mask_y is None: raise ValueError("Label Mask should not be none")
