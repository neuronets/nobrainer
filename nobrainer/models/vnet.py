...
def end_stage(inputs, n_classes = 1, kernel_size=3, activation="relu", padding="SAME"):
    conv = Conv3D(filters= n_classes, kernel_size= kernel_size, activation=activation, padding="SAME")(inputs)
    if n_classes == 1: 
...
def vnet(n_classes = 1, input_shape=(256, 256, 256, 1), kernel_size=3,
    ...
    conv8 = end_stage(conv7, n_classes = n_classes, kernel_size=kernel_size,
