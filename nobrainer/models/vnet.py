...
def down_stage(inputs, filters, kernel_size=3,
    ...
    convd = Conv3D(filters, kernel_size,
                  activation=activation, padding=padding)(inputs)
    convd = GroupNormalization()(convd)
    convd = Conv3D(filters, kernel_size,
                  activation=activation, padding=padding)(convd)
    convd = GroupNormalization()(convd)
...
def up_stage(inputs, skip, filters, kernel_size=3,
    ...
    convu = Conv3D(filters, kernel_size,
                  activation=activation, padding=padding)(merge)
    convu = GroupNormalization()(convu)
    convu = Conv3D(filters, kernel_size,
                  activation=activation, padding=padding)(convu)
    convu = GroupNormalization()(convu)
...
def dropout_vnet(input_shape=(256, 256, 256, 1), kernel_size=3,
                 activation="relu", padding="SAME"):
