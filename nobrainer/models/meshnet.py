...
def meshnet_vwn(n_classes, input_shape, receptive_field=67, filters=71, activation='relu', dropout_rate=0.25, is_mc=False, batch_size=None, name='meshnet_vwn'):
    """Instantiate variational weight MeshNet model.
    ...
    activation: str or optimizer object, the non-linearity to use.
    dropout_rate: float between 0 and 1, the fraction of input units to drop.
    is_mc: bool, whether to apply variational dropout in the variational
        convolution layers.
    batch_size: int, number of samples in each batch. This must be set when
        training on TPUs.
    name: str, name to give to the resulting model object.
    ...
    def one_layer(x, layer_num, dilation_rate=(1, 1, 1)):
        x = VariationalConv3D(filters, kernel_size=(3, 3, 3), padding='same', dilation_rate=dilation_rate, is_mc=is_mc, name='layer{}/vconv3d'.format(layer_num))(x)
        x = layers.BatchNormalization(name='layer{}/batchnorm'.format(layer_num))(x)
        x = layers.Activation(activation, name='layer{}/activation'.format(layer_num))(x)
        x = layers.Dropout(dropout_rate, name='layer{}/dropout'.format(layer_num))(x)
        return x
    ...
    x = VariationalConv3D(filters=n_classes, kernel_size=(1, 1, 1), padding='same', is_mc=is_mc, name='classification/vconv3d')(x)
