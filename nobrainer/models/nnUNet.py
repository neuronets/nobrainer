...
def nnUNet(
    ...
    feature_maps = (
        feature_maps * 2 if feature_maps * 2 < max_feature_maps else max_feature_maps
    )
    ls.append(x)
    ...
    for i in range(num_pool - 1):
        ...
        feature_maps = (
            feature_maps * 2
            if feature_maps * 2 < max_feature_maps
            else max_feature_maps
        )
        ls.append(x)
    ...
    for i in range(len(feature_save)):
        ...
        x = StackedConvLayers(
            x, feature_save[-(i + 1)], kernel_initializer, first_conv_stride=1
        )
        seg_outputs.append(
            Layers.Conv3D(n_classes, (1, 1), use_bias=False, activation="softmax")(x)
        )
...
def StackedConvLayers(x, feature_maps, kernel_initializer, first_conv_stride=2):
    x = ConvDropoutNormNonlin(
        x, feature_maps, kernel_initializer, first_conv_stride=first_conv_stride
    )
