...
class _Conv(base.Layer):
  ...
  def __init__(self, rank,
    ...
    self.a_initializer = tf.constant_initializer(0.0001)#tf.random_normal_initializer(-4.5,0.01)
    ...
    if self.use_bias:
      if self.data_format == 'channels_first':
        ...
        if self.rank == 3:
          ...
          outputs_4d_mean = array_ops.reshape(outputs_mean,
                                         [outputs_shape[0], outputs_shape[1],
                                          outputs_shape[2] * outputs_shape[3],
                                          outputs_shape[4]])
          outputs_4d_std = array_ops.reshape(outputs_std,
                                         [outputs_shape[0], outputs_shape[1],
                                          outputs_shape[2] * outputs_shape[3],
                                          outputs_shape[4]])
          outputs_4d_mean = nn.bias_add(outputs_4d_mean, self.bias_m, data_format='NCHW')
          outputs_4d_var = nn.bias_add(outputs_4d_var, tf.square(self.bias_sigma), data_format='NCHW')
          outputs_mean = array_ops.reshape(outputs_4d_mean, outputs_shape)
          outputs_var = array_ops.reshape(outputs_4d_var, outputs_shape)
      else:
