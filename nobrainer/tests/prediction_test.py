...
def test_predict(tmp_path):
    ...
    # From array.
    y_ = prediction.predict_from_array(
        x, model=model, block_shape=None)
    y_blocks = prediction.predict_from_array(
        x, model=model, block_shape=x.shape)
    y_other = prediction.predict(
        x, model=model, block_shape=None)
    ...
    assert_array_equal(
        y_img.get_fdata(caching='unchanged'),
        y_img_other.get_fdata(caching='unchanged'))

    # From filepath
    y_img2 = prediction.predict_from_filepath(
        path, model=model, block_shape=None)
    y_img2_other = prediction.predict(
        path, model=model, block_shape=None)
    assert isinstance(y_img, nib.spatialimages.SpatialImage)
    assert_array_equal(y_img.shape, x.shape)
    assert_array_equal(y_img.get_fdata(caching='unchanged'), y_)
