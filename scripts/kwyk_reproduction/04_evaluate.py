...
def main() -> None:
    ...
    median_per_class = np.median(class_dice_matrix, axis=0)
    log.info(
    ...
    with open(per_class_csv, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["class_id", "class_name", "mean_dice", "median_dice", "min_dice", "max_dice"])
        for i in range(len(mean_per_class)):
            name = class_names[i] if class_names else str(i + 1)
            col = class_dice_matrix[:, i]
            writer.writerow([i + 1, name, f"{col.mean():.4f}", f"{np.median(col):.4f}", f"{col.min():.4f}", f"{col.max():.4f}"])
