...
def predict_volume(model, img_path, block_shape, mc=False):
    ...
    return _stitch_blocks(block_preds, grid, block_shape, pad, orig_shape, 1)[0].astype(np.int32)
...
def binary_dice(pred, gt):
    pred = (pred > 0).astype(bool); gt = (gt > 0).astype(bool)
...
def train_kwyk(name, n_classes, label_mapping, mc_train, epochs=50):
    ...
    log.info("=== %s: n_classes=%d, mc_train=%s, epochs=%d ===", name, n_classes, mc_train, epochs)
    ...
    with open(WORK_DIR / "kwyk_sanity_manifest.csv") as f:
        for row in csv.DictReader(f):
            ...
            elif row["split"] == "test": val_pairs.append(p)  # use test as val for sanity

    ds = Dataset.from_files(train_pairs, block_shape=block_shape, n_classes=n_classes).batch(32).binarize(label_mapping)
